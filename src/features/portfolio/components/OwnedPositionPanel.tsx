...
import { Loader2, TrendingUp, TrendingDown, Target, XCircle, DollarSign, ChevronUp, ChevronDown, ArrowDownRight, ArrowUpRight, Receipt } from "lucide-react";
import Link from "next/link";
import { apiClient } from "@/lib/apiClient";
import { formatEGP, formatSignedEGP, formatPct, pnlColor } from "@/lib/tradeCalcs";
import { useLanguage } from "@/context/LanguageContext";
import { type DateRange, rangeToFromTo } from "@/lib/rangeToFromTo";
...
/**
 * Rich panel showing the user's relationship with a symbol — currently held *or* previously
 * owned. Renders nothing when the user has no transactions for the symbol.
 *
 * Used inline by /stocks/[symbol]; the standalone /portfolio/positions/[symbol] route now
 * redirects to /stocks/[symbol].
 */
export default function OwnedPositionPanel({ symbol }: { symbol: string }) {
  ...
  // No position record → user has never traded this symbol → render nothing
  if (isError || !data || !data.allTransactions || data.allTransactions.length === 0) {
    return null;
  }
  ...
  const gapFraction = cp != null && be > 0 ? (cp - be) / be : 0;
  const rangeExtent = Math.max(0.25, Math.abs(gapFraction) * 1.2);
  ...
  const sortedGains = [...realizedGains].sort((a, b) => {
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 sm:gap-3 mb-1 flex-wrap">
              <span className="text-gray-400 text-[10px] sm:text-xs font-semibold uppercase tracking-widest">{t("stock.myPosition")}</span>
              {isClosed ? (
          ...
          <ResponsiveContainer width="100%" height={280} className="sm:!h-[400px]">
            <ComposedChart data={chartData} margin={{ top: 8, right: dir === "rtl" ? 16 : 56, left: dir === "rtl" ? 56 : 4, bottom: 0 }}>
              ...
              <Scatter dataKey="sellMark" fill="#10b981" shape={(props: { cx?: number; cy?: number; payload?: { sellMark: unknown; sellPnL: number | null } }) => {
                ...
                const pnlColorHex = pnl != null ? (isProfit ? "#6ee7b7" : "#fca5a5") : "#6ee7b7";
                ...
                return (
                  <g>
                    ...
                    {pnl != null && (
                      <g>
                        <line x1={cx} y1={cy + 12} x2={cx} y2={cy + 24} stroke={pnlColorHex} strokeWidth={1} opacity={0.5} />
                        <rect x={cx - labelW / 2} y={cy + 24} width={labelW} height={18} rx={4} fill={pnlBg} stroke={pnlColorHex} strokeWidth={0.5} opacity={0.95} />
                        <text x={cx} y={cy + 36} textAnchor="middle" fill={pnlColorHex} fontSize={10} fontWeight="700" dominantBaseline="middle">
                          {pnlText}
                        </text>
                      </g>
                    )}
